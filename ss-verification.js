...
// Every platform the SS Verify panel can be pointed at. `action` and
// `buttonLabel` feed the vision-model prompt (see buildBatchPrompt),
// everything else drives the panel UI and player-facing messages. Add a
// new platform by adding an entry here — nothing else needs to change.
const PLATFORMS = {
  instagram: {
    id: 'instagram',
    label: 'Instagram',
    ...
    notButtonLabel: 'Follow',
    usernameLabel: 'Instagram Username',
    usernameHint: 'Instagram handle (e.g. reboundesports or @reboundesports)',
  ...
  youtube: {
    ...
    notButtonLabel: 'Subscribe',
    usernameLabel: 'YouTube Channel',
    usernameHint: 'YouTube handle (e.g. @reboundesports) or channel name',
    usernamePattern: /^[A-Za-z0-9._-]{1,100}$/,
  ...
  twitter: {
    ...
    buttonLabel: 'Following',
    notButtonLabel: 'Follow',
    usernameLabel: 'Twitter / X Username',
    usernameHint: 'Twitter/X handle (e.g. reboundesports or @reboundesports)',
    usernamePattern: /^[A-Za-z0-9_]{1,15}$/,
    urlRegex: /(?:twitter|x)\.com\/([A-Za-z0-9_]+)/i,
  },
};

// One-time migration for guilds set up before the SS Verify panel existed
// (they only had a bare `instagramUsername` string). Runs on every read so
// old data.json files keep working with zero manual steps. Safe to call
// repeatedly — it's a no-op once `ssVerifyPlatform` is set.
function migrateLegacySsSettings(store) {
  if (!store.settings) store.settings = {};
  const s = store.settings;
  if (!s.ssVerifyPlatform && s.instagramUsername) {
    s.ssVerifyPlatform = 'instagram';
    s.ssVerifyUsernames = { ...(s.ssVerifyUsernames || {}), instagram: s.instagramUsername };
  }
  if (!s.ssVerifyUsernames) s.ssVerifyUsernames = {};
  return store;
}

// Resolves the platform + username the panel currently has active, or
// null pieces if setup isn't complete yet.
function getActiveSsVerifyConfig(store) {
  migrateLegacySsSettings(store);
  const platformId = store.settings.ssVerifyPlatform;
  const platform = platformId ? PLATFORMS[platformId] : null;
  const username = platform ? store.settings.ssVerifyUsernames[platformId] : null;
  return { platform, username: username || null };
}
...
function buildBatchPrompt(platform, username, count) {
  return `You are checking ${count} screenshots submitted as proof of ${platform.action} the ${platform.label} account "@${username}". They are attached in order as Screenshot 1, Screenshot 2, etc.
...
- It must look like a genuine ${platform.label} profile/channel screenshot — not an unrelated image, a different app, or an obviously edited/mocked-up button.
- If the button says "${platform.notButtonLabel}" (not "${platform.buttonLabel}"), or the username/handle doesn't match, or you can't clearly tell, mark it NOT verified rather than guessing.
 ...
 */
async function analyzeFollowScreenshots(imageUrls, platform, username) {
  ...
  const parts = [{ text: buildBatchPrompt(platform, username, images.length) }];
...
function buildLogEmbed(message, platform, username, result, thumbnailUrl) {
  return new EmbedBuilder()
    .setTitle(`${platform.emoji} ${platform.label} — Squad Verified`)
    .setColor(0x57F287)
    .setDescription(`${message.author} submitted ${REQUIRED_SCREENSHOT_COUNT} distinct screenshots (themself + 3 teammates) all ${platform.action} **@${username}**.`)
    .addFields(
...
async function handleSsVerifyMessage(message) {
  ...
  if (!platform || !username) {
    await message.reply({
      content: '⚠️ Screenshot verification isn\'t fully set up yet — an admin needs to finish setup in `/ss-verify-panel` (pick a platform and set a username).',
    }).catch(() => {});
    return true;
  }
  ...
  if (existing) {
    await message.reply({
      content: `✅ You're already verified as ${platform.action} **@${username}**.`,
    }).catch(() => {});
    return true;
  }
  ...
  if (images.length === 0) {
    await message.reply({
      content: `${platform.emoji} Post **${REQUIRED_SCREENSHOT_COUNT} screenshots** — yours plus your **3 teammates'** (each a genuinely separate screenshot) — each showing you're ${platform.action} **@${username}** (the "${platform.buttonLabel}" button visible), to get verified. You can send them all in one message or across several — I'll count them as they come in.`,
    }).catch(() => {});
    return true;
  }
  ...
  if (pending.imageUrls.length < REQUIRED_SCREENSHOT_COUNT) {
    const progressEmbed = new EmbedBuilder()
      .setColor(0xFEE75C)
      .setDescription(`${platform.emoji} Got it — **${pending.imageUrls.length}/${REQUIRED_SCREENSHOT_COUNT}** screenshots received. Send ${REQUIRED_SCREENSHOT_COUNT - pending.imageUrls.length} more (yours + teammates') showing you're ${platform.action} **@${username}** to complete verification.`);
  ...
  const result = await analyzeFollowScreenshots(pending.imageUrls, platform, username);
  ...
  if (!result.verified) {
    ...
    const rejectionEmbed = new EmbedBuilder()
      ...
      .setDescription(`❌ **Rejected :**\n${rejectionLines}\n\nMake sure every screenshot clearly shows that profile/channel with the **${platform.buttonLabel}** button. Your progress has been reset — please resend all ${REQUIRED_SCREENSHOT_COUNT}.`)
  ...
  await message.reply({
    content: `✅ Verified! You and your 3 teammates are confirmed as ${platform.action} **@${username}**.`,
  }).catch(() => {});

  const logChannel = await resolveLogChannel(message.guild, store, store.settings.ssVerifyLogChannelId);
  if (logChannel) {
    try {
      await logChannel.send({ embeds: [buildLogEmbed(message, platform, username, result, pending.imageUrls[0])] });
    } catch (err) {
