...
function buildCreateChannelsSubmenuPayload() {
  const embed = new EmbedBuilder()
    ...
    .setColor(0x5865F2)
    .setDescription(
      '**Auto Channels** asks you for a naming format (defaults to `Group {number}`), then creates every group\'s own channel under an auto-managed "🥇 Tournament Groups" category — one channel per group, in order (1, 2, 3...). Use `{number}` for that, or `{letter}` for A, B, C...\n\n' +
...
// no category) using `nameFormat`. "{letter}" is swapped for the group's
// internal letter (A, B, C...) and "{number}" for its 1-based position in
// that order (1, 2, 3...) — so an admin typing "Noble {number}" gets
// "Noble 1", "Noble 2", "Noble 3"... in order, while the groups are still
// tracked internally by letter. If the format uses neither token,
// "-{letter}" is appended so names stay unique across groups. Every
// channel comes out private — only that specific group's own role can see
// it (auto-created here if it doesn't have one yet), so a team registered
// into Group 1 can never see Group 2's channel, and it can't spin up
...
async function handleTournamentWizardButton(interaction) {
  ...
  if (id === 'tourney_wizard_create_channels_auto') {
    ...
    const modal = new ModalBuilder().setCustomId('tourney_wizard_auto_channels_format_modal').setTitle('Group Channel Names');
    const input = new TextInputBuilder()
      .setCustomId('value')
      .setLabel('Name ({number}=1,2,3.. or {letter}=A,B,C..)')
      .setStyle(TextInputStyle.Short)
      .setPlaceholder('Group {number}')
      .setValue('Group {number}')
      .setRequired(true)
      .setMaxLength(80);
    modal.addComponents(new ActionRowBuilder().addComponents(input));
    return interaction.showModal(modal);
...
// Modal submits
// ---------------------------------------------------------------------------
async function handleAutoChannelsFormatModalSubmit(interaction) {
  const store = getTournamentStore(interaction.guildId, interaction.user.id);
  if (!store.tournament) {
    return interaction.reply({ content: '❌ No tournament exists yet.', flags: MessageFlags.Ephemeral });
  }
  ...
  const nameFormat = interaction.fields.getTextInputValue('value').trim() || 'Group {number}';

  await interaction.deferReply({ flags: MessageFlags.Ephemeral });

  // Shares the same "🥇 Tournament Groups" category every auto-created
  // group channel uses, so everything lands in one place regardless of
  // what naming format the admin picked.
...
module.exports = {
  ...
  handleAutoChannelsFormatModalSubmit,
