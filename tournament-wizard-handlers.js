...
async function handleRegisterTeamModalSubmit(interaction) {
  ...
  if (requiredMentions === 0) {
    startRegPending(interaction.user.id, interaction.guildId, { team, tournamentId: tid, selectedPlayerIds: [] });
    return interaction.reply({
      embeds: [buildTeamRegPreviewEmbed({ team, selectedPlayerIds: [] })],
...
async function handleTourneyRegConfirm(interaction) {
  ...
  if (tournament.confirmChannelId) {
    ...
    if (confirmChannel) {
      ...
      await confirmChannel.send(`✅ **${team}** (${interaction.user}) registered into **Group ${letter}**, Slot **${slotNumber}**.${lineup ? `\n👥 ${lineup}` : ''}`).catch(() => {});
      if (justClosed) {
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription(
          ...
          `**Group** — ${letter}\n` +
          `**Slot** — ${slotNumber}\n` +
          (selectedPlayerIds.length ? `**Players** — ${selectedPlayerIds.map(id => `<@${id}>`).join(' ')}\n` : '') +
          (roleWarning || '')
        ),
    ],
    components: [],
