...
// Builds the "who do you want to swap with" dropdown for the self-service
// flow — own team is auto-detected (see findUserTournamentEntry), so this
// only ever needs to list everyone else. tid + the requester's own
// letter/idx ride along in the select's customId since self-service state
// isn't tied to any admin's "active tournament" the way the wizard is.
function buildSwapTargetSelectPayload(tournament, tid, ownEntry) {
  const entries = listAllTeams(tournament)
    .filter(e => !(e.letter === ownEntry.letter && e.idx === ownEntry.idx));
  if (!entries.length) {
    return { error: '❌ There are no other teams to swap with yet.' };
  }
  const trimmed = entries.length > 25;
  const shown = entries.slice(0, 25);

  const select = new StringSelectMenuBuilder()
    .setCustomId(`tourney_wizard_selfservice_swap_select:${tid}:${ownEntry.letter}:${ownEntry.idx}`)
    .setPlaceholder('Select the team to swap groups with')
    .setMinValues(1)
    .setMaxValues(1)
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0x5865F2)
    .setDescription(
      `Your team **${ownEntry.team.team}** is in **Group ${ownEntry.letter}**.\n` +
      'Pick the team you want to trade places with — they\'ll get a confirmation request and nothing changes until they accept.'
    );
  if (trimmed) {
...
async function handleSelfServiceSwapSelect(interaction) {
  const [, tid, ownLetter, ownIdxStr] = interaction.customId.split(':');
  const ownIdx = parseInt(ownIdxStr, 10);
  ...
  const ownGroup = tournament.groups[ownLetter];
  const ownTeam = ownGroup && ownGroup.teams[ownIdx];
  const stillOnTeam = ownTeam && (ownTeam.ownerId === interaction.user.id || (ownTeam.playerIds || []).includes(interaction.user.id));
  if (!stillOnTeam) {
    return interaction.update({ content: "❌ You're no longer on that team — open the Slot-Manager panel and try again.", embeds: [], components: [] });
  }
  ...
  if (!ownTeam.ownerId) {
    return interaction.update({ content: '❌ Your team has no linked owner on record, so a swap can\'t be confirmed — ask an admin for help.', embeds: [], components: [] });
  }
  if (!team.ownerId) {
    return interaction.update({ content: `❌ **${team.team}** has no linked owner on record (added before this feature existed), so there's no one to confirm with.`, embeds: [], components: [] });
  }
  ...
  const swapId = createSwap({
    ...
    letterA: ownLetter,
    teamAName: ownTeam.team,
    ownerAId: ownTeam.ownerId,
    letterB: letter,
    teamBName: team.team,
    ownerBId: team.ownerId,
    requestedBy: interaction.user.id,
    aDecision: 'accept', // picking a team here IS this side's confirmation
  });

  await interaction.update({
    content: `✅ Swap request sent — waiting on **${team.team}**'s owner to respond.`,
    embeds: [], components: [],
  });
...
async function handleSwapDecision(interaction) {
  ...
  // Both accepted — perform the actual swap now. Looked up directly by id
  // (not via getTournamentStore) since whoever requested this might just be
  // a player, not an admin with a "currently open" tournament in the wizard.
  const store = getGuildStore(interaction.guildId);
  const tournament = store.tournaments && store.tournaments[updated.tournamentId];
  if (!tournament) {
    deleteSwap(swapId);
...
async function handleTournamentWizardButton(interaction) {
  ...
  if (id.startsWith('tourney_wizard_selfservice_swap:')) {
    ...
    const payload = buildSwapTargetSelectPayload(tournament, tid, entry);
    if (payload.error) {
...
function buildSlotSelfServicePanelPayload(tid) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      '• Click **Swap Group** to request swapping places with another team — they\'ll need to accept first.\n\n' +
  ...
  const row2 = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(`tourney_wizard_selfservice_swap:${tid}`).setLabel('Swap Group').setEmoji('🔄').setStyle(ButtonStyle.Primary),
  );
