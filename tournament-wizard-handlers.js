...
async function createGroupChannels(interaction, store, { nameFormat, parentId }) {
  ...
  for (let i = 0; i < letters.length; i++) {
    ...
    // role never blocks "create all the channels" outright.
...
function buildManualChannelCreationPayload(data) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      'Creates every group\'s own channel — one per group — under the category below. ' +
...
async function handleTournamentWizardButton(interaction) {
  ...
  if (id === 'tourney_wizard_manual_channels_auto_create') {
    ...
    const { created, missingRole } = await createGroupChannels(interaction, store, {
    ...
    return interaction.editReply({
      content: created.length
        ? `✅ Created under **${category.name}**: ${created.join(', ')}${roleWarning}`
        : `ℹ️ Every group already has a channel (or channel creation failed — check my permissions).${roleWarning}`,
      embeds: [],
      components: [],
    });
  }
