...
function buildTournamentRegisterPanelPayload(tournament) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      tournament.open
        ? 'Click **Register Team** below, enter your team name, owner name and each player\'s IGN + UID (Player 5 is optional), then mention your teammates. You\'ll be auto-assigned to whichever group still has room.'
...
// Registration form — 3 modals shown back-to-back, same layout as the
// verification panel (Discord caps a modal at 5 inputs and this needs 12):
//   Step 1/3: Team name, Owner name, Player 1 IGN + UID
//   Step 2/3: Player 2 IGN + UID, Player 3 IGN + UID
//   Step 3/3: Player 4 IGN + UID, Player 5 IGN + UID  (Player 5 is optional)
// Discord can't open a modal in reply to a modal, so a "Continue" button sits
// between the steps. Answers are staged in the pending-registration store
// (in memory only) — nothing is saved until the player taps Confirm at the
// very end. The player @mentions are picked afterwards (see
// buildMentionPlayersRow) because modals can't hold a user-select.
const MAX_IGN_LENGTH = 30;
const UID_RE = /^[0-9]{5,12}$/;
const REG_STEP_PLAYERS = { 1: [1], 2: [2, 3], 3: [4, 5] };
const REG_LAST_STEP = 3;

function regInput(fieldId, label, draft, { required = true, max = MAX_IGN_LENGTH, placeholder } = {}) {
  ...
  if (draft && draft[fieldId]) input.setValue(String(draft[fieldId]));
  return input;
}

// draft = raw answers typed so far (prefills the fields for "Try Again").
function buildRegisterStepModal(tid, step, draft = {}) {
  const inputs = [];
  if (step === 1) {
    inputs.push(
      regInput('team', 'Team Name', draft, { max: 80 }),
      regInput('owner', 'Team Owner Name', draft, { max: 50 }),
    );
  }
  for (const n of REG_STEP_PLAYERS[step]) {
    const optional = n === 5;
    const tag = optional ? ' (optional)' : '';
    inputs.push(
      regInput(`p${n}_ign`, `Player ${n} - In-Game Name${tag}`, draft, { required: !optional }),
      regInput(`p${n}_uid`, `Player ${n} - Game UID${tag}`, draft, {
        required: !optional, max: 15, placeholder: n === 1 ? 'Numbers only, e.g. 5123456789' : undefined,
      }),
    );
  }
  return new ModalBuilder()
    .setCustomId(`tourney_wizard_register_modal:${tid}:${step}`)
    .setTitle(`Register Team - Step ${step}/${REG_LAST_STEP}`)
    .addComponents(inputs.map(input => new ActionRowBuilder().addComponents(input)));
}

// Returns an error message for this step's answers, or null if they're fine.
function validateRegistrationStep(tournament, draft, step) {
  const get = key => String(draft[key] || '').trim();

  if (step === 1) {
    const team = get('team');
    if (!team) return '❌ Team name is required.';
    if (isBanned(tournament, team)) return `❌ **${team}** is banned from registering.`;
    if (isDuplicateTeam(tournament, team)) return `❌ A team named **${team}** is already registered.`;
    if (!get('owner')) return '❌ Owner name is required.';
  }

  for (const n of REG_STEP_PLAYERS[step]) {
    const ign = get(`p${n}_ign`);
    const uid = get(`p${n}_uid`);
    if (n === 5 && !ign && !uid) continue; // Player 5 is optional
    if (n === 5 && (!ign || !uid)) {
      return '❌ Player 5 needs both an In-Game Name and a UID — or leave both blank if there is no Player 5.';
    }
    if (!ign) return `❌ Player ${n}'s In-Game Name is required.`;
    if (!UID_RE.test(uid)) return `❌ Player ${n}'s Game UID must be numbers only (5-12 digits).`;
  }

  // Every player needs their own UID (checked against everyone entered so far).
  const seen = new Set();
  for (let n = 1; n <= Math.max(...REG_STEP_PLAYERS[step]); n++) {
    const uid = get(`p${n}_uid`);
    if (!uid) continue;
    if (seen.has(uid)) return `❌ UID **${uid}** is entered more than once — every player needs their own UID.`;
    seen.add(uid);
  }
  return null;
}

// "**Owner** — x" + "**P1** — ign (uid)" ... lines for a team (empty string if
// the team has none, e.g. registrations made before these fields existed).
function formatTeamDetailLines(team) {
  const lines = [];
...
// Step failed validation: the answers are already staged in the pending
// store, so "Try Again" reopens that same step prefilled — like the
// verification panel, one typo never costs the player the whole form.
function rejectRegistrationForm(interaction, tid, step, message) {
  return interaction.reply({
    content: `${message} Tap **Try Again** to fix it — your other answers are kept.`,
    components: [new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`tourney_wizard_register_retry:${tid}:${step}`).setLabel('Try Again').setEmoji('🔁').setStyle(ButtonStyle.Primary)
    )],
    flags: MessageFlags.Ephemeral,
  });
}

function registerContinueRow(tid, nextStep) {
  const label = nextStep === 2 ? 'Continue to Player 2 & 3' : 'Continue to Player 4 & 5';
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`tourney_wizard_register_continue:${tid}:${nextStep}`).setLabel(label).setEmoji('➡️').setStyle(ButtonStyle.Primary)
  );
}
...
async function handleTournamentWizardButton(interaction) {
  ...
  if (id.startsWith('tourney_wizard_register_team:')) {
    ...
    return interaction.showModal(buildRegisterStepModal(tid, 1));
  }

  // Public: "Try Again" (after a validation error) and "Continue" (between
  // the form's steps) both reopen a step's modal, prefilled with whatever the
  // player has typed so far. Both live in the pending-registration store.
  if (id.startsWith('tourney_wizard_register_retry:') || id.startsWith('tourney_wizard_register_continue:')) {
    const [, tid, stepRaw] = id.split(':');
    const step = Number(stepRaw);
    if (![1, 2, 3].includes(step)) {
      return interaction.reply({ content: `❌ Something went wrong. ${RESTART_HINT}`, flags: MessageFlags.Ephemeral });
    }
    const tournament = getTournamentById(interaction.guildId, tid);
    if (!tournament) {
      return interaction.reply({ content: '❌ This tournament no longer exists.', flags: MessageFlags.Ephemeral });
    ...
    return interaction.showModal(buildRegisterStepModal(tid, step, pendingEntry.data.draft));
  }
  ...
  if (id.startsWith('tourney_wizard_selfservice_my_groups:')) {
    ...
    const lineup = (team.playerIds && team.playerIds.length)
      ...
      : (team.players || []).join(' ');
    const details = formatTeamDetailLines(team);
    return interaction.reply({
    ...
    for (const [letter, group] of Object.entries(store.tournament.groups).sort(([a], [b]) => a.localeCompare(b))) {
      group.teams.forEach((t, idx) => {
        ...
        const row = sheet.addRow({
          ...
          owner: t.ownerName || '', ownerId: t.ownerId || '',
...
// Public team registration, part 1 — the 3-step details form (team name,
// owner name, player IGN + UID; see buildRegisterStepModal). Each step just
// validates + stages its answers; after step 3 the player mentions their
// teammates and the actual slot assignment happens when they hit Confirm
// (see handleTourneyRegSelectPlayers / handleTourneyRegConfirm below).
async function handleRegisterTeamModalSubmit(interaction) {
  const [, tid, stepRaw] = interaction.customId.split(':');
  const step = [1, 2, 3].includes(Number(stepRaw)) ? Number(stepRaw) : 1;
  const store = getGuildStore(interaction.guildId);
  const tournament = store.tournaments && store.tournaments[tid];
  ...
  // This step's raw answers.
  const answers = {};
  const fieldIds = step === 1 ? ['team', 'owner'] : [];
  for (const n of REG_STEP_PLAYERS[step]) fieldIds.push(`p${n}_ign`, `p${n}_uid`);
  for (const fieldId of fieldIds) answers[fieldId] = interaction.fields.getTextInputValue(fieldId).trim();

  // Stage them (step 1 starts a fresh session; later steps add to it).
  if (step === 1) {
    startRegPending(interaction.user.id, interaction.guildId, { tournamentId: tid, draft: answers });
  } else {
    const pendingEntry = getRegPending(interaction.user.id);
    if (!pendingEntry || pendingEntry.data.tournamentId !== tid || !pendingEntry.data.draft) {
      return interaction.reply({ content: `❌ Your session expired or was interrupted. ${RESTART_HINT}`, flags: MessageFlags.Ephemeral });
    }
    updateRegPending(interaction.user.id, { draft: { ...pendingEntry.data.draft, ...answers } });
  }
  const draft = getRegPending(interaction.user.id).data.draft;

  const problem = validateRegistrationStep(tournament, draft, step);
  if (problem) {
    return rejectRegistrationForm(interaction, tid, step, problem);
  }

  if (step < REG_LAST_STEP) {
    return interaction.reply({
      content: step === 1
        ? '✅ Step 1 saved. Continue to enter Player 2 & 3 details.'
        : '✅ Step 2 saved. Continue to enter Player 4 & 5 details (Player 5 is optional).',
      components: [registerContinueRow(tid, step + 1)],
      flags: MessageFlags.Ephemeral,
    });
  }

  // Last step done — assemble the final team details.
  const team = draft.team.trim();
  const ownerName = draft.owner.trim();
  const playerIgns = [];
  const playerUids = [];
  for (let n = 1; n <= 5; n++) {
    const ign = String(draft[`p${n}_ign`] || '').trim();
    const uid = String(draft[`p${n}_uid`] || '').trim();
    if (ign && uid) { playerIgns.push(ign); playerUids.push(uid); }
  }
  ...
  if (requiredMentions === 0) {
    startRegPending(interaction.user.id, interaction.guildId, { team, ownerName, playerIgns, playerUids, tournamentId: tid, selectedPlayerIds: [] });
    return interaction.reply({
      embeds: [buildTeamRegPreviewEmbed({ team, ownerName, playerIgns, playerUids, selectedPlayerIds: [] })],
      components: [tourneyConfirmRow()],
      flags: MessageFlags.Ephemeral,
    });
  }

  startRegPending(interaction.user.id, interaction.guildId, { team, ownerName, playerIgns, playerUids, tournamentId: tid });
...
async function handleTourneyRegConfirm(interaction) {
  ...
  const { team, ownerName, playerIgns, playerUids, selectedPlayerIds = [], tournamentId } = pendingEntry.data;
  const store = getGuildStore(interaction.guildId);
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription(
          ...
          `**Group** — ${letter}\n` +
          `**Slot** — ${slotNumber}\n` +
          (formatTeamDetailLines({ ownerName, playerIgns, playerUids }) ? `${formatTeamDetailLines({ ownerName, playerIgns, playerUids })}\n` : '') +
